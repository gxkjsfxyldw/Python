...
def mall():#商场********************************************************
    ...
    def order():
        ...
        windows = Tk()
        windows.title("订单信息")
        windows.geometry('480x650+750+200')
        frm5 = Frame(windows, )
        frm5.config(bg='white', width=480, height=650, )
        frm5.place(x=0, y=0, )

        lables = Label(text="国内承运快递：", font=('楷体', 14))
        lables.place(x=90, y=75)

        lable = Label(text="请完善您的收货订单", fg='red', font=('楷体', 14), )
        lable.place(x=150, y=15)

        canvas = Canvas(windows, height=50, width=160)

        image_file = PhotoImage(file=r'sf.png')
        canvas.create_image(0, 0, anchor='nw', image=image_file)

        canvas.pack()
        canvas.place(x=225, y=60)

        lables = Label(text="订单编号：", font=('楷体', 14))
        lables.place(x=90, y=130)

        texts = Text(windows, width=15, height=1, font=('楷体', 14))
        texts.place(x=230, y=130)
        texts.insert("end", sf)  # 订单编号

        a = Label(frm5, text='收件人姓名：', font=('楷体', 14), ).place(x=90, y=180)
        p = Label(frm5, text='    手机号：', font=('楷体', 14), ).place(x=90, y=230)
        m = Label(frm5, text='      地址：', font=('楷体', 14), ).place(x=90, y=280)

        ae = Entry(frm5, text='1')
        ae.place(x=235, y=180)
        pe = Entry(frm5, text='2')
        pe.place(x=235, y=230)
        me = Entry(frm5, text='3')
        me.place(x=235, y=280)

        text2 = Text(frm5, width=30, height=10, font=('楷体', 14), )  # 用来放选项内容
        text2.place(x=90, y=350)

        if ad > 0:
            a1 = str(ad)

            text2.insert("end", '耐克篮球数量:' + a1 +'个'+'\n')  # 必须统一数据类型
        if ad1 > 0:
            a1 = str(ad1)

            text2.insert("end", '李宁篮球数量:' + a1 + '个' + '\n')  # 必须统一数据类型
        if ad2 > 0:
            a1 = str(ad2)

            text2.insert("end", '准者篮球数量:' + a1 + '个' +'\n')  # 必须统一数据类型
        if ad3 > 0:
            a1 = str(ad3)

            text2.insert("end", '威尔胜篮球数量:' + a1 + '个' + '\n')  # 必须统一数据类型
        if ad4 > 0:
            a1 = str(ad4)

            text2.insert("end", '斯伯丁篮球数量:' +a1 + '个'+  '\n')  # 必须统一数据类型
        if ad5 > 0:
            a1 = str(ad5)

            text2.insert("end", '安踏篮球数量:' + a1 + '个'+ '\n')  # 必须统一数据类型
    ...
    # 创建四个复选框  按钮 加购
    plus = Button(frm2, text="加入购物车", width=10, font=('黑体', 8), bg="red", fg="white", height=1, command=incres).place(
        x=115, y=245)
    plus1 = Button(frm2, text="加入购物车", width=10, font=('黑体', 8), bg="red", fg="white", height=1, command=incres1).place(
        x=272, y=245)
    plus2 = Button(frm2, text="加入购物车", width=10, font=('黑体', 8), bg="red", fg="white", height=1, command=incres2).place(
        x=425, y=245)
    plus3 = Button(frm2, text="加入购物车", width=10, font=('黑体', 8), bg="red", fg="white", height=1, command=incres3).place(
        x=115, y=447)
    plus4 = Button(frm2, text="加入购物车", width=10, font=('黑体', 8), bg="red", fg="white", height=1, command=incres4).place(
        x=272, y=447)
    plus5 = Button(frm2, text="加入购物车", width=10, font=('黑体', 8), bg="red", fg="white", height=1, command=incres5).place(
        x=425, y=447)

    # 创建四个复选框  按钮 加购
    pluss = Button(frm2, text="-", width=1, font=('黑体', 8), bg="blue", fg="white", height=1, command=decres).place(x=50,
                                                                                                                   y=245)
    pluss1 = Button(frm2, text="-", width=1, font=('黑体', 8), bg="blue", fg="white", height=1, command=decres1).place(
        x=207, y=245)
    pluss2 = Button(frm2, text="-", width=1, font=('黑体', 8), bg="blue", fg="white", height=1, command=decres2).place(
        x=360, y=245)
    pluss3 = Button(frm2, text="-", width=1, font=('黑体', 8), bg="blue", fg="white", height=1, command=decres3).place(
        x=50, y=447)
    pluss4 = Button(frm2, text="-", width=1, font=('黑体', 8), bg="blue", fg="white", height=1, command=decres4).place(
        x=207, y=447)
    pluss5 = Button(frm2, text="-", width=1, font=('黑体', 8), bg="blue", fg="white", height=1, command=decres5).place(
        x=360, y=447)
    ...
    Button(frm2, text="退出商场", width=10, height=2, command=tuichu).place(x=670, y=400)
