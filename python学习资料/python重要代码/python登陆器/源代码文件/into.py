...
def pt():
    ...
    main_windows.iconbitmap("111.ico")  # 窗口小图标
    photo = PhotoImage(file=r'44.png')
    Label(main_windows, text='', image=photo, ).place(x=-5, y=-140)  # 插入背景及规定起始位置

    theLB = Listbox(main_windows)  # 创建一个空列表
    theLB.place(x=150,y=50)#----------------------------------------------------------------
    # 往列表里添加数据
    result = 0
    for i in a:
        t = str(a[result])
    ...
    qut = Button(main_windows, text="返回", bg="#1bb7ea", fg="white", width=10, command=show)
    qut.place(x=180,y=250)
...
def dlt():
    ...
    frm1.config(bg='white', width=450, height=320, )
    frm1.place(x=0, y=0, )
    main_windows.title('注销账号')

    photo = PhotoImage(file=r'6.png')
    aaa = Label(frm1, text='', image=photo, ).place(x=5, y=5)  # 插入背景及规定起始位置
    ...
    def show():
        ...
        elif  choses==2 and Acount!=zhanghao:
              tkinter.messagebox.showwarning("温馨提示", "普通用户只能删除本次登录的账号")
              main_windows.destroy()
              dlt()
        elif choses==2 and Acount==zhanghao:
    ...
    ini = Button(main_windows, text="删除",bg="royal blue", width=10, command=show).place(x=120, y=170)
    qut = Button(main_windows, text="返回",bg="royal blue", width=10, command=fanhui).place(x=240, y=170)
...
def c():  # 建立函数块  这样在别的文件引用时就不会马上就弹出了，就可以等点击的时候再弹出
    ...
    photo = PhotoImage(file=r'6.png')
    aaa = Label(frm1, text='', image=photo, ).place(x=5, y=5)  # 插入背景及规定起始位置
    top = Label(frm1, text='账号为数字0-9，密码为字母或数字', bg="white", ).place(x=250, y=35)
    account = Label(frm1, text='账号：',
                    justify=LEFT,  # 左对齐
                    compound=CENTER,  # 设置文本和图像的混合模式a
                    font=('楷体', 12),
    ...
    account.place(x=90, y=90)  # x：水平距离 ； y：垂直距离
    lack.place(x=90, y=145)
    double_lack.place(x=90, y=195)

    account_Entry = Entry(frm1, text='1', )
    lack_Entry = Entry(frm1, show='*')
    double_lack = Entry(frm1, show="*")
    # 以下是规定输入框所在的位置
    account_Entry.place(x=210, y=90)
    lack_Entry.place(x=210, y=145)
    double_lack.place(x=210, y=195)
    ...
    def show():
        ...
        elif  choses==2 and int(act)!=zhanghao:
              tkinter.messagebox.showwarning("温馨提示", "普通用户只能修改本次登录的账号")
              main_windows.destroy()
              c()
    ...
    def fanhui():
        main_windows.destroy()
        chose()
    ini = Button(frm1, text="确定", width=10, bg="royal blue", fg="white", command=show).place(x=120, y=245)
    qut = Button(frm1, text="返回", width=10, bg="royal blue", fg="white", command=fanhui).place(x=240, y=245)
    main_windows.mainloop()
...
def e():  # 建立函数块  这样在别的文件引用时就不会马上就弹出了，就可以等点击的时候再弹出
    ...
    photo = PhotoImage(file=r'6.png')
    Label(frm1, text='', image=photo, ).place(x=5, y=5)  # 插入背景及规定起始位置

    top = Label(frm1, text='账号为数字0-9，密码字母或者数字', bg="white").place(x=250, y=35)
    ...
    ini = Button(main_windows, text="注册", width=10, bg="royal blue", fg="white", command=show).place(x=120, y=245)
    qut = Button(main_windows, text="退出", width=10, bg="royal blue", fg="white", command=main_windows.destroy).place(x=240,y=245)
...
def chose():
    ...
    t1=tk.Button(main_windows, bg="RoyalBlue1", fg="white", text='修改密码', font=('楷体', 12), anchor='c', width=21, height=2,
              command=two)
    t2=tk.Button(main_windows, bg="RoyalBlue2", fg="white", text='注销账号', font=('楷体', 12), anchor='c', width=21, height=2,
              command=three)
    t3=tk.Button(main_windows, bg="RoyalBlue3", fg="white", text='查看账号', font=('楷体', 12), anchor='c', width=21, height=2,
              command=four)
    t4=tk.Button(main_windows, bg="RoyalBlue4", fg="white", text='退出登录', font=('楷体', 12), anchor='c', width=21, height=2,
              command=five)

    t1.place(x=135, y=30)
    t2.place(x=135, y=80)
    t3.place(x=135, y=130)
    t4.place(x=135, y=180)
    main_windows.mainloop()
...
def w():
    ...
    photo = PhotoImage(file=r'55.png')
    aaa = Label(frm1, text='', image=photo, ).place(x=-38, y=-20)
    ...
    tv=tk.Radiobutton(main_windows, text="管理员登录", variable=v, value=1,bg="#1bb7ea")
    tv.place(x=240,y=230)

    tv2=tk.Radiobutton(main_windows, text="普通用户登录", variable=v, value=2,bg="#1bb7ea")
    tv2.place(x=120,y=230)
    ...
    ini = Button(frm1, text="登录", bg="#1bb7ea", fg="white", width=10, command=show).place(x=120, y=270)
    qut = Button(frm1, text="退出", bg="#1bb7ea", fg="white", width=10, command=main_windows.destroy).place(x=240, y=270)
    main_windows.mainloop()
